...
import numpy as np
from conv_utils import conv_output_length, deconv_length
...
class MultiLength(layers.Layer):
    def __init__(self, seg=True, **kwargs):
        super(MultiLength, self).__init__(**kwargs)
        self.seg = seg
    ...
    def get_config(self):
        config = {'seg': self.seg}
        base_config = super(MultiLength, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class MultiMask(layers.Layer):
    def __init__(self, n_class=2, **kwargs):
    ...
    def call(self, inputs, **kwargs):
        if type(inputs) is list:
            assert len(inputs) == 2
            input, mask = inputs
            B, H, W, C, A = input.get_shape()
            mask = tf.expand_dims(mask, axis=4)
            mask_list = []
            for i in range(A):
                mask_list.append(mask)
            mask_tensor = tf.concat(mask_list, axis=4)
            mask_tensor_reshape = tf.reshape(mask_tensor, [-1, H, W, C*A])
            input_tensor_reshape = tf.reshape(input, (-1, H, W, C*A))
            masked = mask_tensor_reshape * input_tensor_reshape

            return masked

        else:
            B, H, W, C, A = inputs.get_shape()
            length_inputs = tf.norm(inputs, axis=-1)
            prediction = tf.argmax(length_inputs, axis=-1)
            #prediction_one_hot = keras.utils.np_utils.to_categorical(prediction, self.n_class)
            prediction_one_hot = tf.one_hot(prediction, self.n_class)
            mask = tf.expand_dims(prediction_one_hot, axis=4)
            mask_list = []
            for i in range(A):
                mask_list.append(mask)
            mask_tensor = tf.concat(mask_list, axis=4)
            mask_tensor_reshape = tf.reshape(mask_tensor, [-1, H, W, C * A])
            input_tensor_reshape = tf.reshape(inputs, [-1, H, W, C * A])
            masked = mask_tensor_reshape * input_tensor_reshape
    ...
    def compute_output_shape(self, input_shape):
        ...
        B, H, W, C, A = input_shape
        output_shape = [B, H, W, C*A]
        return tuple(output_shape)

    def get_config(self):
        base_config = super(MultiMask, self).get_config()
